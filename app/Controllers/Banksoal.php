...
class Banksoal extends BaseController
{
    ...
    public function edit($id)
    {
        if ($id == "add") {
            ...
            $soal = $builder->get()->getResult();
            // d($soal);

            $indexName = ['pertanyaan', 'opsi_a', 'opsi_b', 'opsi_c', 'opsi_d', 'opsi_e', 'pembahasan'];

            foreach ($soal as $soalBuilder) {
                foreach ($indexName as $name) {
                    $soalBuilder->$name = SoalHelper::parseJsonToHtmlView($soalBuilder->$name, $name);
                }
            }
            $data['soal'] = $soal;
    ...
    public function edit_soal($id_soal)
    {
        ...
        foreach ($indexName as $name) {
            $soalBuilder[$name] = SoalHelper::parseJsonToHtml($soalBuilder[$name], $name);
        }
    ...
    public function update_soal($id_soal)
    {
        ...
        foreach ($indexName as $name) {
            ...
            // dd($file);
            // rubah ke json
            $post[$name] = array_map(function ($block) {
                return json_decode($block, true);
            }, $post[$name]);
            // d($post[$name]);
            // hapus yang tidak ada file upload atau src nya
            $post[$name] = array_filter($post[$name], function ($block) use ($file) {
                if ($block['type'] == "text") {
                    return true;
            ...
            foreach ($post[$name] as $i => $block) {
                ...
                if ($block['type'] == 'image' || $block['type'] == 'audio') {
                    ...
                    $post[$name][$i]['src'] = $newFileName;
                    unset($post[$name][$i]['fileIndex']);
                }
            }
            // dd($post, $file);
            //ekstract src
            ...
            $image_db_array_src =
                ...
                }, $soalBuilder[$name]), function ($block) {
                    return $block != null;
                }));
